...
import 'package:image/image.dart' as img; // 이미지 회전 보정에 필요
...
class _ReceiptScannerHomePageState extends State<ReceiptScannerHomePage> {
  ...
  Future<void> _pickImageFromCamera() async {
    ...
    final XFile? image = await _picker.pickImage(
      ...
      imageQuality: 100, // 고화질로 설정
      preferredCameraDevice: CameraDevice.rear,
    );
    if (image != null) {
      // 이미지 회전 자동 보정 (EXIF)
      File rotatedImage = await _fixImageRotation(File(image.path));
      setState(() {
        _image = rotatedImage;
        _isLoading = true;
      });
  ...
  Future<File> _fixImageRotation(File imageFile) async {
    // 패키지: exif, image 필요 (pubspec.yaml에 추가)
    try {
      final bytes = await imageFile.readAsBytes();
      final originalImage = img.decodeImage(bytes);
      if (originalImage == null) return imageFile;
      final exifData = await readExifFromBytes(bytes);
      var orientationValue = exifData['Image Orientation']?.printable;
      int? orientation;
      if (orientationValue != null) {
        if (orientationValue.contains('180')) orientation = 3;
        else if (orientationValue.contains('90 CW')) orientation = 6;
        else if (orientationValue.contains('90 CCW')) orientation = 8;
      }
      img.Image fixedImg = originalImage;
      if (orientation == 3) {
        fixedImg = img.copyRotate(originalImage, angle: 180);
      } else if (orientation == 6) {
        fixedImg = img.copyRotate(originalImage, angle: 90);
      } else if (orientation == 8) {
        fixedImg = img.copyRotate(originalImage, angle: -90);
      }
      final fixedFile = await imageFile.writeAsBytes(img.encodeJpg(fixedImg));
      return fixedFile;
    } catch (e) {
      return imageFile;
    }
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['responses'] != null && 
          ...
          setState(() {
            ...
            _selected = List<bool>.filled(refinedItems.length, true);
            _isLoading = false;
          });
        } else {
          setState(() {
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              Checkbox(
                                value: _selected[i],
                                onChanged: (v) => setState(() => _selected[i] = v ?? true),
                              ),
                              Expanded(
                                child: Text(
                                  item.name,
                              ...
                              IconButton(
                                ...
                                onPressed: item.quantity > 1
                                    ? () => setState(() => _receiptItems[i] = ReceiptItem(name: item.name, quantity: item.quantity - 1, price: item.price))
                                    : null,
                              ),
                              Text('${item.quantity}'),
                              IconButton(
                                icon: Icon(Icons.add_circle_outline),
                                onPressed: () => setState(() => _receiptItems[i] = ReceiptItem(name: item.name, quantity: item.quantity + 1, price: item.price)),
                              ),
                            ],
                          ),
                        );
                      ...
                      Align(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: _sendSelectedItems,
                          icon: Icon(Icons.send),
                          label: Text('선택 항목 보내기'),
                        ),
                      )
                    ],
                  ),
...
// 상호작용: 선택 항목을 앱으로 전달하는 콜백 (Navigator.pop으로 반환)
extension _SendSelected on _ReceiptScannerHomePageState {
  void _sendSelectedItems() {
    final selected = <ReceiptItem>[];
    for (int i = 0; i < _receiptItems.length; i++) {
      if (i < _selected.length && _selected[i]) {
        selected.add(_receiptItems[i]);
      }
    }
    if (selected.isEmpty) {
